...
const TOC: FC<Props> = ({ selector }) => {
  ...
  return (
    <aside className="sticky block top-10">
      <nav
        className={`${
          expand ? "max-h-fit" : "max-h-[350px]"
        } border-l border-[#3c3c431f] no-scrollbar transition-max_height duration-500 overflow-y-scroll`}
      >
        <ul ref={listWrapperRef} className="!pl-0">
          <li className="flex items-center gap-2 pl-10 tracking-widest text-lg text-gray-700 uppercase mb-4 font-extrabold">
              ...
              <li
                ...
                className={`my-4 cursor-pointer line-clamp-1 ${
                  currentHeadingID === heading.dataset.id
                    ? "font-bold text-primary border-l-2 border-primary"
                    : "text-gray-800"
                }`}
                onClick={() => {
                  window.scrollTo({
